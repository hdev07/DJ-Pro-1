...
export class Song {
  ...
  public static async from(url: string = "", search: string = "") {
    ...
    if (isYoutubeUrl) {
      ...
      result ? null : console.log(`No hay resultados para ${search}`); // This is for handling the case where no results are found (spotify links for example)

      if (!result) {
        let err = new Error(`Ho hay resultados para ${search}`);
        err.name = "NoResults";
